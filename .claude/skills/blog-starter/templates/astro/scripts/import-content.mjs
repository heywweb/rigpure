...
// {{categories}} — le skill régénère ce tableau depuis blog.config.json.categories
const CATS = [
  { slug: 'guides', name: 'Guides', description: 'In-depth guides, how-tos and deep dives.' },
  ...
  { slug: 'setup', name: 'Setup', description: 'Recommendations, tools and gear.' },
];
...
for (const cat of CATS) {
  ...
  for (const file of fs.readdirSync(dir).filter((f) => f.endsWith('.md'))) {
    ...
    if (data.game) games.set(slug, data.game);
    ...
    articles.push({ ...data, slug, category: cat.slug, body_md: content, tagList });
  }
}
...
lines.push(`INSERT INTO authors (id,slug,name,bio,avatar,role,twitter) VALUES (1,'{{authorSlug}}','{{author}}','{{authorBio}}',NULL,'Editor',NULL);`);
...
for (const a of articles) {
  ...
  lines.push(
    `INSERT INTO articles (id,slug,title,dek,body_md,body_html,category_id,game_id,author_id,cover,cover_alt,cover_source,reading_time,rating,verdict,pros,cons,tier,published_at,updated_at,views,featured,meta_description,og_image,canonical,noindex,draft) VALUES (` +
    `${aid},${q(a.slug)},${q(a.title)},${q(a.dek)},${q(a.body_md)},${q(bodyHtml)},${catId[a.category]},` +
    `${gameId[a.slug] ?? 'NULL'},1,${q(a.cover)},${q(a.cover_alt)},${q(a.cover_source)},` +
